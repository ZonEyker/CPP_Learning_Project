...
WaypointQueue Tower::reserve_terminal(Aircraft& aircraft)
{
    ...
    const auto waypoint_termNum = airport.reserve_terminal(aircraft);
    if (!waypoint_termNum.first.empty())
    {
        reserved_terminals.emplace(&aircraft, waypoint_termNum.second);
    }
    return waypoint_termNum.first;
}

void Tower::delete_reserved_terminal(Aircraft& aircraft)
{
    assert(&aircraft);
